...
$(document)
    .on('click', '.movie-tile', function (event) {
        var trailerYouTubeId = $(this)
            .attr('data-trailer-youtube-id')
...
href = "#" > Bret 's Favorites</a> < /div> < /div> < /div> < /div> < div class =
    "container" > {
        movie_tiles
    } < /div> < /body> < /html>
''
...
def create_movie_tiles_content(movies): #The HTML content
for this section of the page
content = ''
for movie in movies: #Extract the youtube ID from the url
...
# Append the tile
for the movie with its content filled in
...
def open_movies_page(movies): #Create or overwrite the output file
...
# open the output file in the browser( in a new tab,
    if possible)
